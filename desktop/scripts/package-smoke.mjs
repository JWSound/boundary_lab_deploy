...
const rotatedPose = constrainCabinetPoses(
  ...
  // Rotate toward the neighbor: this cabinet extends behind its source origin.
...
assert.equal(parsedProject.audience_planes[0].columns, 24);
assert.equal(parsedProject.audience_planes[0].displayMode, "real_pressure");
assert.equal(parsedProject.audience_planes[0].pressureScalePa, 24);
...
delete incompleteProject.audience_planes[0].heatmapMinimumDb;
